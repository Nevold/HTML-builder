...
const { mkdir, readdir, copyFile, rm, access, stat, readFile } = require('fs/promises');
const fs = require('fs');
...
async function mergeStylesCss() {
  ...
  const promisesFiles = files.map(async (file) => {
    ...
    if (fileStatus.isFile() && path.parse(currentFilePath).ext.slice(1) === 'css') {
...
async function mergeTemplateHtml() {
  ...
  const promisesFiles = files.map(async (file) => {
    ...
    const fileStatus = await stat(currentFilePath);
    if (fileStatus.isFile() && path.parse(currentFilePath).ext.slice(1) === 'html') {
      const readStream = fs.createReadStream(currentFilePath, 'utf-8');
      ...
      readStream.on('end', () => {
        ...
        indexHtml = indexHtml.split(currHtmlTemp).join(templateSourceObj[currHtmlTemp]);
